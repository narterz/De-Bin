...
>(
    ...
    async (file, thunkAPI) => {
        ...
        try {
            ...
            console.error(`Server error occurred while converting file ${file.metadata.fileName}. See backend for logs. ${errorMessage}`)
...
>(
    ...
    async ({ fileState, fileObj }, thunkAPI) => {
        ...
        try {
            ...
            const errorMessage = err instanceof Error ? err.message : String(err);
            console.error(`Server error occurred while uploading file ${fileState.metadata.fileName}. See backend for logs. ${errorMessage}`)
            return thunkAPI.rejectWithValue({ status: 'failure', error: "Server error has occurred." });
        }
    }
)
...
>(
    ...
    async (file, thunkAPI) => {
        ...
        try {
            ...
            const errorMessage = err instanceof Error ? err.message : String(err);
            console.error(`Unknown server error occurred while removing file ${file.metadata.fileName}. Error: ${errorMessage}`)
            return thunkAPI.rejectWithValue({ status: 'failure', error: "Server error has occurred." });
        }
    }
)
...
>(
    ...
    async (file, thunkAPI) => {
        ...
        void console.debug(`[downloadFile] Processing download of file ${fileName}`)
        try {
            ...
            void console.debug(`[downloadFile] Successfully downloaded file: ${fileName}`);
        } catch (error) {
            void console.error(`[downloadFile] Failed to download file ${fileName}. Error: ${error}`);
    ...
    extraReducers: (builder) => {
        ...
        builder.addCase(convertFile.pending, (state, action) => {
            ...
            builder.addCase(convertFile.fulfilled, (state, action) => {
                ...
                const newFileState: FileState = action.payload.fileState;
                ...
                void console.debug(`File ${formerFileName} successfully converted from ${formerFileExt} to ${newFileState.fileConversions?.conversion}`);
        ...
        builder.addCase(uploadFileToBackend.pending, (state, action) => {
            ...
            builder.addCase(uploadFileToBackend.fulfilled, (state, action) => {
                ...
                void console.debug(`File ${fileName} has successfully been uploaded to backend.`)
        ...
        builder.addCase(removeFileFromBackend.fulfilled, (state, action) => {
            ...
            void console.debug(`Successfully removed file ${fileToRemove?.metadata.fileName}`)
